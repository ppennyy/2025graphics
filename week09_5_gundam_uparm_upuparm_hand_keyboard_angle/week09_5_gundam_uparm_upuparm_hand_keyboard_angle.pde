//week09_5_gundam_uparm_upuparm_hand_keyboard_angle
...
int ID=0;//關節的代碼之後會用angle[ID]來改變值
void keyPressed(){
  if(key=='1')ID=1;
  if(key=='2')ID=2;
}
void mouseDragged(){
  angle[ID]+=mouseX-pmouseY;//X方向的移動，可改變某個關節的角度
}
void draw(){
  background(204);
  translate(200,300);
  ...
  pushMatrix();
    ...
    pushMatrix();
      ...
      //println(mouseX/10.0,-mouseY/10.0);//印出適合的 數值ex. 4.5 , -16.9
      shape(hand1,0,0);
    popMatrix();
  popMatrix();
}
